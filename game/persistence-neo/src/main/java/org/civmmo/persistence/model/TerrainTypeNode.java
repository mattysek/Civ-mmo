 ...
 * @author Mattysek
 */
public class TerrainTypeNode extends TerrainType {
    private final Node node;
    
    public TerrainTypeNode(Node node)
    {
        this.node = node;
        this.node.addLabel(DynamicLabel.label("TerrainType"));
    }
    ...
    {
        return o instanceof TerrainTypeNode &&
                this.node.equals( ( (TerrainTypeNode)o ).getUnderlyingNode() );
    }

    @Override
    public String toString()
    {
        return "TerrainType[" + this.getId() + "]";
    }
    ...
    public CombatModifier getCombatModifier() {
        return NodeHelper.getSingleRelationship(node,RelationshipType.TERRAIN_TYPE_COMBAT_MODIFIER,e -> new CombatModifierNode(e));
    }

    @Override
    public void setCombatModifier(CombatModifier combatModifier) {
        NodeHelper.setSingleRelationship(node,combatModifier,RelationshipType.TERRAIN_TYPE_COMBAT_MODIFIER, e -> ((CombatModifierNode)e).getUnderlyingNode());
    }

    @Override
    public List<Tile> getOfTiles() {
        return NodeHelper.getRelationship(node,RelationshipType.TERRAIN_TYPE_TILE,e -> new TileNode(e));
    }

    @Override
    public void setOfTiles(List<Tile> ofTiles) {
        NodeHelper.setRelationship(node,ofTiles,RelationshipType.TERRAIN_TYPE_TILE, e -> ((TileNode)e).getUnderlyingNode());
    }
}
